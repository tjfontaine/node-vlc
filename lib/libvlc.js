...
var search_path = [process.cwd()];

switch (os.platform()) {
  case 'darwin':
    search_path.push('/Applications/VLC.app/Contents/MacOS/lib/');
    search_path.push(path.join(process.env.HOME, 'Applications/VLC.app/Contents/MacOS/lib'));
    break;
  case 'win32':
    if (process.env['ProgramFiles(x86)']) {
      search_path.push(path.join('',
...
try {
  var v = new FFI.Library('libvlc', { search_path: search_path }, {
    libvlc_get_version: ['string', []],
  });
} catch (e) {
  throw new Error("Failed to find LibVLC, make sure it's installed, if you're on linux you may only have a versioned .so");
}

var vlcversion = v.libvlc_get_version().split(' ')[0];

if (!semver.satisfies(vlcversion, '>= 2.0.1'))
  throw new Error("Requires LibVLC >= 2.0.1, you have: " + vlcversion);

var MediaStatsPtr = require('./libvlc_types').MediaStatsPtr;

module.exports = new FFI.Library('libvlc', { search_path: search_path }, {
    ...
    libvlc_log_iterator_next: ['pointer', ['pointer', 'pointer']],
    libvlc_module_description_list_release: ['void', ['pointer']],
    libvlc_audio_filter_list_get: ['pointer', ['pointer']],
    libvlc_video_filter_list_get: ['pointer', ['pointer']],
    ...
    libvlc_video_get_track_description: ['pointer', ['pointer']],
    ...
    libvlc_audio_get_track_description: ['pointer', ['pointer']],
